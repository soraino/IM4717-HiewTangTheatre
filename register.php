...
if (
    ...
    || !isset($_POST['creditCardNumber']) || !isset($_POST['cvv'])
...
if ($phoneNumberResults->num_rows > 0) {
    $DbValidation = false;
?>
<script>
...
if ($cardNumberResults->num_rows > 0) {
    $DbValidation = false;
?>
<script>
...
$INSERT_CARD_QUERY = "INSERT INTO  f34ee.Card ( CardNumber , Name , CVV , ExpiryDate ) VALUES ('" . trim($_POST['creditCardNumber']) . "','" . trim($_POST['creditCardName']) . "','" . trim($_POST['cvv']) . "','" . trim(urldecode($_POST['cardExpiry'])) . "');";
    ...
    if ($cardInsertResult) {
        ...
        if ($userIdResult->num_rows > 0) {
            ...
            if ($cardHolderResult) {
                ...
                $to = 'f34ee@localhost';
                $subject = 'Welcome to HiewTang Theatres';
                $message = "Hi ".$_POST['name'].",\nWelcome to HiewTang Theatres. You can now book movie tickets with us!\n\nHappy Movie Viewing!";
                $headers = 'From: message@hiewtangtheatres.com' . "\r\n".'Reply-To: f34ee@localhost' . "\r\n".'X-Mailer: PHP/'. phpversion();
                mail($to,$subject,$message,$headers,'-f34ee@localhost');
